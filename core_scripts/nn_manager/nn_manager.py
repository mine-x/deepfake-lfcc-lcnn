...
def f_run_one_epoch(args,
    ...
    for data_idx, (data_in, data_tar, data_info, idx_orig) in \
        ...
        if optimizer is not None:
            ...
            # LR warmup: linearly ramp LR from ~0 to target over first N steps
            warmup_steps = getattr(args, 'warmup_steps', 0)
            if warmup_steps > 0:
                global_step = epoch_idx * len(data_loader) + data_idx
...
def f_train_wrapper(args, pt_model, loss_wrapper, device, \
    ...
    # Give loss wrapper access to model (needed for OC-Softmax)
    if hasattr(loss_wrapper, 'set_model'):
        loss_wrapper.set_model(pt_model)
    ...
    if checkpoint is not None:
        _partial_load = getattr(pt_model, 'use_oc_softmax', False) or \
                        getattr(pt_model, 'use_se_attention', False)
        if type(checkpoint) is dict:
            ...
            if cp_names.state_dict in checkpoint:
                ...
                pretrained_dict = nii_nn_tools.f_state_dict_wrapper(
                    checkpoint[cp_names.state_dict], flag_multi_device)
                if _partial_load:
                    # Filter out keys with mismatched shapes (e.g. output layer)
                    model_dict = pt_model.state_dict()
                    compatible = {k: v for k, v in pretrained_dict.items()
                                  if k in model_dict and v.shape == model_dict[k].shape}
                    skipped = set(pretrained_dict.keys()) - set(compatible.keys())
                    if skipped:
                        nii_display.f_print("Partial load — skipped %d keys "
                                            "with shape mismatch: %s" %
                                            (len(skipped), str(skipped)))
                    model_dict.update(compatible)
                    pt_model.load_state_dict(model_dict)
                else:
                    pt_model.load_state_dict(pretrained_dict)

            # load optimizer state (skip if architecture changed)
            if cp_names.optimizer in checkpoint and \
               not args.ignore_optimizer_statistics_in_trained_model \
               and not _partial_load:
    ...
    for epoch_idx in range(start_epoch, epoch_num):
        ...
        # update training augmentor epoch for per-epoch randomness
        if hasattr(train_dataset_wrapper, 'm_dataset') \
           and hasattr(train_dataset_wrapper.m_dataset, 'm_augmentor') \
           and train_dataset_wrapper.m_dataset.m_augmentor is not None \
           and hasattr(train_dataset_wrapper.m_dataset.m_augmentor, 'set_epoch'):
            train_dataset_wrapper.m_dataset.m_augmentor.set_epoch(epoch_idx)
