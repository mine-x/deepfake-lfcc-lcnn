...
def f_args_parsed(argument_input = None):
    ...
    parser.add_argument('--warmup-steps', type=int, default=0,
                        help='Number of steps for linear LR warmup (0=disabled)')
    parser.add_argument('--label-smoothing', type=float, default=0.0,
                        help='Label smoothing epsilon (0.0=disabled, e.g. 0.1)')
    parser.add_argument('--curriculum', action='store_true', default=False,
                        help='Enable curriculum noise scheduling (easy to hard over epochs)')
    parser.add_argument('--pos-weight', type=float, default=-1.0,
                        help='Weight for positive (bonafide) class to handle imbalance. '
                             '-1.0=disabled, 8.7=inverse class freq ratio')
    parser.add_argument('--mixup-alpha', type=float, default=0.0,
                        help='Mixup Beta distribution parameter (0.0=disabled, 0.2=typical). '
                             'Blends pairs of training samples and labels for regularization.')
    parser.add_argument('--oc-softmax', action='store_true', default=False,
                        help='Use OC-Softmax loss instead of BCELoss. '
                             'Learns compact bonafide boundary with angular margin.')
    parser.add_argument('--emb-dim', type=int, default=64,
                        help='Embedding dimension for OC-Softmax (default: 64). '
                             'Ignored when --oc-softmax is not set.')
    parser.add_argument('--oc-r-real', type=float, default=0.9,
                        help='OC-Softmax margin for bonafide samples (default: 0.9)')
    parser.add_argument('--oc-r-fake', type=float, default=0.2,
                        help='OC-Softmax margin for spoof samples (default: 0.2)')
    parser.add_argument('--oc-alpha', type=float, default=20.0,
                        help='OC-Softmax scaling factor (default: 20.0)')
    parser.add_argument('--se-attention', action='store_true', default=False,
                        help='Add SE (Squeeze-and-Excitation) attention blocks to LCNN. '
                             'Recalibrates channel importance after each MaxPool layer.')
    parser.add_argument('--freq-mask-width', type=int, default=0,
                        help='Max width for frequency feature masking (0=disabled, 10=typical). '
                             'Randomly masks low/high/random freq bands in LFCC during training.')
    ...
    #######
    # augmentation (inference-only)
    mes = 'Augmentation condition for robustness evaluation (inference only). '
    mes += 'e.g. noise_office_10dB, noise_cafe_0dB, short_3s, short_5s. '
    mes += 'Default: empty string (no augmentation).'
    parser.add_argument('--augment', type=str, default='', help=mes)

    # training augmentation
    mes = 'Enable noise augmentation during training. '
    mes += 'Randomly applies ambient/babble noise at 10-25dB SNR to 50%% of samples. '
    mes += 'Default: disabled.'
    parser.add_argument('--train-augment', action='store_true', default=False,
                        help=mes)

    mes = 'Subdirectory under noise_clips/ containing noise clips. '
    mes += 'e.g. "train" (6 clips for training), "eval" (6 unseen clips for evaluation). '
    mes += 'Default: train.'
    parser.add_argument('--noise-subdir', type=str, default='train', help=mes)
