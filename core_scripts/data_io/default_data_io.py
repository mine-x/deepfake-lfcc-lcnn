...
def _data_len_reader(file_path):
    ...
    if file_ext == '.wav':
        sr, data = nii_wav_tk.waveReadAsFloat(file_path)
        length = data.shape[0]
    elif file_ext == '.flac': # Updated to include debug print
        try:
            sr, data = nii_wav_tk.flacReadAsFloat(file_path)
            length = data.shape[0]
        except Exception as e:
            print("[ERROR] FAILED FLAC FILE:", file_path, flush=True)
            raise e
    elif file_ext == '.txt':
...
class NIIDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.m_augmentor = None
        if global_arg is not None:
            if getattr(global_arg, 'augment', '') and getattr(global_arg, 'inference', False):
                # inference-time augmentation (specific condition)
                from augment import Augmentor
                noise_subdir = getattr(global_arg, 'noise_subdir', 'selected')
                self.m_augmentor = Augmentor(global_arg.augment, noise_subdir=noise_subdir)
            elif getattr(global_arg, 'train_augment', False) and not getattr(global_arg, 'inference', False):
                # training-time augmentation (random noise, 50% probability)
                from augment import TrainingAugmentor
                noise_subdir = getattr(global_arg, 'noise_subdir', 'selected')
                curriculum = getattr(global_arg, 'curriculum', False)
                self.m_augmentor = TrainingAugmentor(noise_subdir=noise_subdir, curriculum=curriculum)
    ...
    def f_post_data_process(self, in_data, out_data, seq_info, idx):
        ...
        if self.m_opt_wav_handler > 0:
            ...
            in_data_out, out_data_out, seq_info_out = \
                in_data_n, out_data_n, tmp_seq_info

        else:
            in_data_out, out_data_out, seq_info_out = in_data, out_data, seq_info

        # on-the-fly augmentation (after silence handling)
        if self.m_augmentor is not None:
            wav_1d = in_data_out[:, 0]
            augmented = self.m_augmentor(wav_1d, seq_info_out.seq_name)

            if augmented is not None:
                in_data_out = np.expand_dims(augmented, axis=1)
                if in_data_out.shape[0] != seq_info_out.length:
                    seq_info_out = nii_seqinfo.SeqInfo(
                        in_data_out.shape[0], seq_info_out.seq_name,
                        seq_info_out.seg_idx, seq_info_out.start_pos,
                        seq_info_out.info_id)
            else:
                print(f"[augment] SKIP {seq_info_out.seq_name}: "
                      f"shorter than target")

        return in_data_out, out_data_out, seq_info_out, idx
