...
More can be found in [./01_project/README](./01_project/README)
...
3. If waveform is in WAV format, please change the line `input_exts = ['.flac']` to `input_exts = ['.wav']` in 01_project/baseline_LA/config.py and 01_project/baseline_LA/config_auto.py
...
./01_project | project directories, and each folder correspond to one model for one dataset
./01_project/\*/\*/main.py | script to load data and run training and inference
./01_project/\*/\*/model.py | model definition based on Pytorch APIs
./01_project/\*/\*/config.py | configurations for training/val/test set data
